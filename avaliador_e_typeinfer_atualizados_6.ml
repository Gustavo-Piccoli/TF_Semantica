...
let rec typeinfer (gamma: ambiente_tipo) (e:expr) : tipo  = match e with
  ...
  | ExApp(e1,e2) -> (match typeinfer gamma e1 with
      | TyFn(t1, t2) ->  if (typeinfer gamma e2) = t1 then t2 else raise TypeError
      | _ -> raise TypeError)
  ...
  | ExList(e1, e2) -> (match typeinfer gamma e2 with
      | TyList(t1) -> if (typeinfer gamma e1) = t1 then TyList(t1) else raise TypeError
      | _ -> raise TypeError)
...
let rec eval (gamma:ambiente_valor) (e:expr) : valor = match e with
  ...
  | ExApp(e1,e2) -> (match eval gamma e1 with
        | VClos(x,e3,novo_gamma) -> eval (update novo_gamma x (eval gamma e2)) e3
        | VRclos(f,x,e3,novo_gamma) -> eval (update (update novo_gamma x (eval gamma e2)) f (eval gamma e1)) e3
        | _ -> raise BugTypeInfer)
  ...
  (* Funcao Declarada Recursiva *)
  | ExLetRec(f,TyFn(t1,t2),ExFn(x,tx,e1), e2) when t1 = tx -> eval (update gamma f (VRclos(f,x,e1,gamma))) e2
...
let rec valor_para_tipo (v: valor) : tipo = match v with
  ...
  | VNil(t) -> TyList(t)
  | VList(v1,v2) -> (if (valor_para_tipo v1) = (valor_para_tipo v2) then TyList(valor_para_tipo v1) else raise BugTypeInfer)
  | VJust(v) -> TyMaybe(valor_para_tipo v)
  | VNothing(t) -> TyMaybe(t)
  | VClos(x,e,gamma) -> TyFn(valor_para_tipo(eval gamma e), valor_para_tipo(eval gamma (ExVar(x)))) (*ERRADO*)
  | VRclos(f,x,e,gamma) -> TyFn(valor_para_tipo (eval gamma e), valor_para_tipo (eval gamma (ExVar(x)))) (*ERRADO*)
